document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("solveRoom3").addEventListener("click", async () => {
    // added 'async' to the event listener function in order to turn the function into an asynchronous function as I want to use the 'await' keyword inside the function.
...
function findMostRecentBook(books) {
  ...
  return books.reduce((mostRecent, book) =>
    new Date(book.published) > new Date(mostRecent.published)
      ? book
      : mostRecent
  ); // in order to find the most recent book, I used > (greater than) instead of < (less than) in the comparison of publication dates.
}
...
function findIntersection(setA, setB) {
  ...
  ); // used the spread operator (...) to convert the sets to arrays. the findIntersection function will now use set operations and array methods to find the intersection between the two input sets, ensuring that only the elements present in both sets will be/are included in the output.
...
async function navigateLabyrinth(directions) {
  for (let direction of directions) {
    // this loop iterates through the directions array and logs each step to the console.
    // 🪲 Bug: No delay
